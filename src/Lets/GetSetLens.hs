...
modify ::
  ...
  -> a
  -> a
modify lns f v = set lns v $ f $ get lns v
...
fmodify ::
  ...
  -> a
  -> f a
fmodify lns ff a = set lns a <$> ff (get lns a)
...
mapL k = Lens (\m newv -> Map.alter (const newv) k m) (Map.lookup k)
...
setL ::
  ...
  k
  -> Lens (Set k) Bool
setL k = Lens (\s newv -> bool Set.delete Set.insert newv k s) (Set.member k)
...
product ::
  ...
  -> Lens c d
  -> Lens (a, c) (b, d)
product lx ly = Lens (\(a,b) (c,d) -> (set lx a c, set ly b d)) (\(a,b) -> (get lx a, get ly b))
...
choice ::
  ...
  -> Lens b x
  -> Lens (Either a b) x
choice lx ly = Lens (\a b -> bimap (\aa -> set lx aa b) (\aa -> set ly aa b) a)
                    (either (get lx) (get ly))
